...
/* $Id$ */
    ...
    if (!$id) { Header('Location: ' . $phpgw->link('/projects/index.php',"sort=$sort&order=$order&query=$query&start=$start&filter=$filter&cat_id=$cat_id")); }
    ...
    if (($submit) && (! $error) && (! $errorcount)) { $t->set_var('message',lang('Project x x has been updated !',$num,$title)); }
