...
/* $Id$ */
    ...
    if ($query) {
    $querymethod = " (phpgw_p_invoice.num like '%$query%' OR phpgw_p_invoice.date like '%$query%' OR phpgw_p_invoice.sum like '%$query%') ";
	if ($project_id) { $phpgw->db->query("select count(*) from phpgw_p_invoice where $querymethod AND project_id=$project_id"); }
	else { $phpgw->db->query("select count(*) from phpgw_p_invoice where $querymethod"); }
	...
	if ($phpgw->db->f(0) == 1) { $t->set_var('lang_showing',lang('your search returned 1 match')); }
        ...
        if ($project_id) { $phpgw->db->query("select count(*) from phpgw_p_invoice WHERE project_id=$project_id"); }
        else { $phpgw->db->query("select count(*) from phpgw_p_invoice"); }
	$phpgw->db->next_record();
...
// ----------------------- end header declaration -----------------------------
    ...
    if ($project_id) {
	$sql = "SELECT phpgw_p_invoice.id as id,phpgw_p_invoice.num,title,phpgw_p_invoice.date,sum,"
	    . "phpgw_p_invoice.project_id as pid,phpgw_p_invoice.customer "
	    . "FROM phpgw_p_invoice,phpgw_p_projects WHERE "
	    . "phpgw_p_invoice.project_id=phpgw_p_projects.id AND phpgw_p_projects.id='$project_id' "
