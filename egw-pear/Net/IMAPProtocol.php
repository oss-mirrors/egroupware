...
class Net_IMAPProtocol {
     ...
     *
     * @param string The userid to authenticate as or "$userid\000$adminuser" and admin-password as password.
     * @param string The password to authenticate with.
     * @param string The cmdID.
     *
     * @return array Returns an array containing the response
    ...
    function _authPLAIN($uid, $pwd, $cmdid)
    {
        ...
        // only prefix $uid with \0, if it not already contain one
        $auth_str = base64_encode((strpos($uid, chr(0)) !== false ? '' : chr(0)).$uid.chr(0).$pwd );
