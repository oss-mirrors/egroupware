<?php
	...
	* @package egwical
	* @version 0.9.30-ng-a1 first version for egwical napi3
	\**************************************************************************/
	...
	class vcalinfolog extends boinfolog
	{
	  ...
	  function vcalinfolog()
	  {
		...
		error_log("warning class: infolog.vcalinfolog call DEPRECATED," .
				  "\nplease rewrite your code to use egwical.boinfolog_vtodos class" .
				  "\n now temporary code fix used ");
		$this->infhnd =& CreateObject('egwical.boinfolog_vtodos');
		$this->infhnd->set_rsc($this);

		if ($this->infhnd == false){
		  error_log('vcalinfolog constructor: couldnot add boinfolog resource to egwical rschnd: FATAL');
		  return false;
		}
	  }
