<?php
	...
	*/
	...
	if ($submit)
	{
		...
		{
			$GLOBALS['egw']->preferences->add ($GLOBALS['egw_info']["flags"]["currentapp"], $internal, $$internal);
		}
		...
		Header('Location: '.$GLOBALS['egw']->link('/preferences/index.php'));
	...
	$p =& CreateObject ('phpgwapi.Template', $GLOBALS['egw']->common->get_tpl_dir ('filemanager'));
	$templates = array (
		'pref'			=> 'pref.tpl',
	...
	$var = array (
		...
		'action_url'	=>	$GLOBALS['egw']->link ('/' . $GLOBALS['egw_info']['flags']['currentapp'] . '/preferences.php'),
	...
	{
		unset ($checked);
		if ($GLOBALS['egw_info']["user"]["preferences"]["filemanager"][$internal])
		{
			$checked = 1;
		}
	...
	while (list ($internal, $value_array) = each ($other_dropdown))
	{
		...
		while (list ($num, $value) = each ($value_array))
		{
			...
			$options .= html_form_option ($value, $value, $GLOBALS['egw_info']["user"]["preferences"]["filemanager"][$internal] == $value, True);
		}
