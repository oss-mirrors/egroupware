...
class module_calendar extends Module 
{
		...
		$this->post = array(
			'prevmonth' => array(
				'type' => 'image',
				'src' => $GLOBALS['egw']->common->image('phpgwapi','left')
			),
			'nextmonth' => array(
				'type' => 'image',
				'src' => $GLOBALS['egw']->common->image('phpgwapi','right')
			)
		);
		...
		$date = $GLOBALS['egw']->datetime->gmtdate($arguments['timestamp']);
		$month = $date['month'];
		$day = $date['day'];
		$year = $date['year'];
		...
		$weekstarttime = $GLOBALS['egw']->datetime->get_weekday_start($year,$month,1);
		...
		$linkdata['month'] = $GLOBALS['egw']->common->show_date($date['raw'],'m');
		$linkdata['year'] = $GLOBALS['egw']->common->show_date($date['raw'],'Y');

		$month = '<a href="' .$GLOBALS['egw']->link('/index.php',$linkdata)  . '" class="minicalendar">' . lang($GLOBALS['egw']->common->show_date($date['raw'],'F')).' '.$GLOBALS['egw']->common->show_date($date['raw'],'Y').'</a>';

		$var = Array(
			'cal_img_root'		=>	$GLOBALS['egw']->common->image('calendar','mini-calendar-bar'),
		...
		if(!$t->get_var('daynames'))
		{
			...
			{
				$var = Array(
					'dayname'	=> '<b>' . substr(lang($GLOBALS['egw']->datetime->days[$i]),0,2) . '</b>',
					'day_image'	=> ''
				);
				$this->output_template_array($t,'daynames','mini_day',$var);
			}
		}
		$today = date('Ymd',$GLOBALS['egw']->datetime->users_localtime);
		unset($date);
		for($i=$weekstarttime + $GLOBALS['egw']->datetime->tz_offset;date('Ymd',$i)<=$monthend;$i += (24 * 3600 * 7))
		{
			unset($var);
			$daily = $this->set_week_array($i - $GLOBALS['egw']->datetime->tz_offset,$cellcolor,$weekly);
			@reset($daily);
			while(list($date,$day_params) = each($daily))
			{
				...
				$linkdata['date']= $date;
				$str = '<a href="' .$GLOBALS['egw']->link('/index.php',$linkdata) .'" class="' .$day_params['class'] .'">' .$day .'</a>';
...
//			unset($p);
	...
	function set_week_array($startdate,$cellcolor,$weekly)
	{
		for ($j=0,$datetime=$startdate;$j<7;$j++,$datetime += 86400)
		{
			...
			if($weekly)
			{
				$cellcolor = $GLOBALS['egw']->nextmatchs->alternate_row_color($cellcolor);
			}
			...
			if($holidays)
			{
				...
				{
					$day_image = ' background="'.$GLOBALS['egw']->common->image('calendar','mini_day_block').'"';
				}
			}
			...
			{
				$extra = ' bgcolor="'.$GLOBALS['egw_info']['theme']['cal_today'].'"';
				$class = ($appts?'b':'').'minicalendar';
				$day_image = ' background="'.$GLOBALS['egw']->common->image('calendar','mini_day_block').'"';
			}

			if($this->bo->check_perms(EGW_ACL_ADD))
			{
				$new_event = True;
			}
			else
