   ...
   *  option) any later version.                                              *
   ...
   if($_POST['save'])
   {
      ...
      if (!$cat_group_id)
      {
         ...
         . $GLOBALS['phpgw']->db->quote($cat_group['cat_id'],'int') . ", "
         . $GLOBALS['phpgw']->db->quote($cat_group['account_id'],'int'). ")",__LINE__,__FILE__);
      }
      else
      {
         $GLOBALS['phpgw']->db->query("update phpgw_tts_categories_groups "
            . " set cat_id=". $GLOBALS['phpgw']->db->quote($cat_group['cat_id'],'int'). ", "
            . " account_id=". $GLOBALS['phpgw']->db->quote($cat_group['account_id'],'int')
            . " WHERE cat_group_id=".$GLOBALS['phpgw']->db->quote($cat_group_id,'int'),
            __LINE__,__FILE__);
