...
//  $GLOBALS['phpgw']->template->set_block('index', 'tts_ticket_id_unread', 'tts_ticket_id_unread');
  ...
  if ($searchfilter) 
  {
    ...
    $extradata = 'searchfilter=' . $searchfilter;
  } else {
    $extradata = '';
  }
  ...
  else
  {
	...
	$total++;
      ...
      $subject = str_replace(array('\\\'','\\"','\\\\'),array("'",'"','\\'),$db->f('ticket_subject'));
			...
			// (erics, 22.05.2006) removed clipping of subject line.
...
# MSc: due date
  ...
  $GLOBALS['phpgw']->template->set_var('left', $GLOBALS['phpgw']->nextmatchs->left('/tts/index.php',$start,$total,$extradata));
  $GLOBALS['phpgw']->template->set_var('right',$GLOBALS['phpgw']->nextmatchs->right('/tts/index.php',$start,$total,$extradata));
