...
//	$GLOBALS['phpgw']->template->set_var('tts_head_dateopened', $GLOBALS['phpgw']->nextmatchs->show_sort_order($sort,'',$order,'/tts/index.php',lang('Date opened')));
	...
	else
	{
		while ($db->next_record())
		{
			...
			$GLOBALS['phpgw']->template->set_var('tts_t_assignedto', $db->f('ticket_assignedto')?$GLOBALS['phpgw']->common->grab_owner_name($db->f('ticket_assignedto')):lang('None'));
			$GLOBALS['phpgw']->template->set_var('tts_t_user',$GLOBALS['phpgw']->common->grab_owner_name($db->f('ticket_owner')));
