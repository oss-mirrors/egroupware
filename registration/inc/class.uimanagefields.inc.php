	...
	*  option) any later version.                                              *
	...
	class uimanagefields
	{
		...
		function uimanagefields ()
		{
			...
			$phpgw->nextmatches = CreateObject ('phpgwapi.nextmatchs');
		...
		function admin ($message = NULL)
		{
			...
			$var = array (
				'action_url'	=> $phpgw->link ($this->base_url, 'menuaction=registration.uimanagefields.submit'),
				'message'	=> $message,
			...
			$row_color = $phpgw->nextmatches->alternate_row_color ($row_color);
			$var = Array(
				'row_off'	=> $phpgw->nextmatches->alternate_row_color ($row_color)
			);
			$p->set_var ($var);

			$row_color = $phpgw->nextmatches->alternate_row_color ($row_color);
			$var = Array(
				'row_on'	=> $phpgw->nextmatches->alternate_row_color ($row_color)
			);
			$p->set_var ($var);
		...
		function submit ()
		{
			...
			Header ('Location: ' . $phpgw->link ($this->base_url, 'menuaction=registration.uimanagefields.admin&message=Updated'));
		}
	}
?>