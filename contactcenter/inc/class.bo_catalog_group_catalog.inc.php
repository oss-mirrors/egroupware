  ...
  *  option) any later version.                                               *
  \***************************************************************************/
	...
	class bo_catalog_group_catalog extends abo_catalog
	{
	 	...
	 	*/
		function bo_catalog_group_catalog(& $bo_contactcenter, & $catalog)
		{
		...
		*/
		function find($what, $rules, $other)
		{
			if ($ldap_info = $this->catalog['ldap'])
			{
				$meta_catalog =& CreateObject('contactcenter.bo_global_ldap_catalog',$ldap_info['id_source'], $ldap_info['context']);
				return $meta_catalog->find($what, $rules, $other);
			}
			...
			while(list(, $new_catalog) = each($this->catalog['sub_branch']))
			{
				...
				$temp_res = $this->bo_contactcenter->catalog->find($what, $rules, $other);
