<?php
	...
	****************************************************************************/
	...
	/** @page pageicalsrvvfreebusyfeatures Currently implemented VFREEBUSY handling in icalsrv
	* <PRE>
...
@verbatim
  ...
  $devicetype = 'all';               
  $calhnd =& CreateObject( 'icalsrv.bocalupdate_vfreebusy',
                           $boc,
                           $devicetype,
                           $fbreq );
...
@verbatim
   ...
   $vcalstr1 = icalsrv_resourcehandler::render_velt2vcal($vfreebusies);
@endverbatim 
	...
	*
	 ...
	 * icalsrv-resourcehandler, because in the vfreebusy multiple events are converted
	 ...
	 */
	class bocalupdate_vfreebusy extends icalsrv_resourcehandler
	{
		/**
		* @private
		* @var boolean
		...
		*/
		function _provided_vfreebusy2eventFields()
		{
			return array(
				...
				'COMMENT'	=> array('fn_rn' => ''),
				'FREEBUSY'  => array(
					'fn_rns'=> array('start','end','recur_type', 'recur_exception')
				)
			);
		}
		...
		function export_vfreebusy(&$events)
		{
			...
			if(in_array('FREEBUSY',$vfExportFields))
			{
				...
				foreach($events as $event)
				{
					...
					$avalue = $this->ecu->mki_v_FREEBUSY(
						$this->ecu->st_dst_patch($event['start']),
		...
		* $deviceType field of the icalsrv_resourcehandler. See there
		...
		* browser, a syncml client etc.)  the icalsrv_resoucehandler
