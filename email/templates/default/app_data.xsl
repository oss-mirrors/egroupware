	...
	<xsl:template match="msg_list_dsp">
		<tr>
			...
			<td align="center">
				...
				<xsl:if test="is_flagged != '0'">
					<xsl:value-of disable-output-escaping="yes" select="../flagged_img"/>
				</xsl:if>
				<xsl:if test="is_answered != '0'">
					<xsl:value-of disable-output-escaping="yes" select="../answered_img"/>
				</xsl:if>
				<xsl:if test="is_draft != '0'">
					<xsl:value-of disable-output-escaping="yes" select="../draft_img"/>
				</xsl:if>
				<xsl:if test="is_deleted != '0'">
					<xsl:value-of disable-output-escaping="yes" select="../deleted_img"/>
				</xsl:if>
			</td>
			<td align="left">
				<xsl:choose>
