<?php
	...
	* Inc., 59 Temple Place, Suite 330, Boston, MA  02111-1307  USA			*
	...
	/*!
	@class spell
	@abstract  Handles spell checking for email messages. Give it all necessary data and it handles the rest. All class params are 
	private unless an OOP style access function is provided.
	@param $max_suggest (int) maximum number is suggestions to show in a combobox
	@param $spell_svc (object) the spell service access class, a dummy module if pspell is not compiled in
	...
	@param $subject (string) use OOP access functions ->set_subject and ->get_subject.
	@param $preserve_tokens (array) Scheduled For Depreciation, but still used. A list of GPC vars we typically want to preserve 
			thru the spellchecking process. However, it is too limiting to had a hard-coded list, need to allow more flexibility.
	@param $preserve_vars (array) use OOP access functions ->set_preserve_var and ->get_preserve_vars. Currently, 
			you can set any vars you want, but only those in the $preserve_tokens will be preserved as hiddenvars. NEEDS FIXING. 
	@param $body_orig  string use OOP access functions ->set_body_orig and ->get_body_orig.
			This is record of the original message before any spell check changes.
	@param $body_display  (PRIVATE) string ACCESS ONLY use OOP access functions  ->get_body_display.
			Scheduled for depreciation because the function "prep_body_display" directly returns this data and 
			there us no readon to keep it stored in this param. "prep_body_display" is quite useful, do not 
			limit it by bounding it to this param.
	@param THERE ARE MORE...
	@discussion  Provide this class with any of the above vars that you have. Subject and Body will be 
	spellchecked, but the other data is needed because this class handles the rest of the logic from spell 
	checking, to user review of spellcheck, to applying the users choices to the body, only then is the 
	message handed off to another object, either class send for sending, or class compose if the user wants to 
	edit the text some more. Calling process MUST set any vars it wants preserved through the spellcheck 
