...

{
	...
	$file = Array(
		'JiNN Main' => array(
			'link' => $GLOBALS['egw']->link('/index.php','menuaction=jinn.uiuser.index'),
			'icon' => 'mini_navbar',
			'text' => 'JiNN Main'
		),
	);
	...
	if($GLOBALS['local_bo']->site['site_id'] && $GLOBALS['local_bo']->site_object['object_id'])
	{
		$object = Array(
			'Browse current object' => array(
				'link' => $GLOBALS['egw']->link('/index.php','menuaction=jinn.uiu_list_records.display'),
				'icon' => 'browse',
				'text' => 'List record(s) in current object'
			),
			'Export current object' => array(
				'link' => $GLOBALS['egw']->link('/index.php','menuaction=jinn.uiu_export.export'),
				'icon' => 'filesave',
	...
	if($GLOBALS['local_bo']->site['site_id'] && $GLOBALS['local_bo']->site_object['object_id'] && $GLOBALS['local_bo']->site_object['max_records']!=1)
	{
		$object = Array(
			'Add multiple records' => array(
				...
				'icon'=>'new',
				'link'=>$GLOBALS['egw']->link('/index.php','menuaction=jinn.uiu_edit_record.multiple_entries&insert=yes')
			),
			'Add new entry' => array(
				'text'=>'Add new entry',
				'icon'=>'new',
				'link'=>$GLOBALS['egw']->link('/index.php','menuaction=jinn.uiu_edit_record.display_form')
			)
		);
		$file=array_merge($file,$object);
	}
	...
	if($GLOBALS['local_bo']->last_where_string)
	{
		$last_record = Array(
			...
			(
				'link' => $GLOBALS['egw']->link('/index.php','menuaction=jinn.uiu_edit_record.display_form&where_string='.$GLOBALS['local_bo']->last_where_string),
	...
	if($GLOBALS['local_bo']->site['site_id'] && $GLOBALS['local_bo']->site['object_scan_prefix'] != '')
	{
		$site = Array(
			'Scan for new Objects' => array(
				'link' => $GLOBALS['egw']->link('/index.php','menuaction=jinn.bouser.scan_new_objects&where_key=site_id&where_value='.$GLOBALS['uiuser']->bo->site['site_id']),
	...
	$file = Array(
		'General Preferences' => array(
			'link' => $GLOBALS['egw']->link('/index.php','menuaction=preferences.uisettings.index&appname=jinn'),
			'icon' => 'configure',
	...
	if($GLOBALS['local_bo']->site_object['object_id'])
	{
		$conf = Array(
			'Configure this Object List View'=> array(
				'link' => $GLOBALS['egw']->link('/index.php','menuaction=jinn.uiuser.config_objects'),
	...
	if(!$GLOBALS['egw_info']['user']['apps']['admin'] && $GLOBALS['local_bo'] && count($GLOBALS['local_bo']->so->get_sites_for_user2($GLOBALS['egw_info']['user']['account_id'])) > 0)
	{
		$menu_title = lang('Administration');
		$file = Array(
			'Access Rights' => array(
				'link' => $GLOBALS['egw']->link('/index.php','menuaction=jinn.uiacl.main_screen'),
				'text' => 'Access Rights',
				'icon' => 'groupevent'
			)
		);
	...
	elseif($GLOBALS['egw_info']['user']['apps']['admin'])
	{
		...
		$file = Array(
			'Global Configuration' => array(
				'link' => $GLOBALS['egw']->link('/index.php','menuaction=admin.uiconfig.index&appname=' . $appname),
			...
			'Add Site' => array(
				'link' => $GLOBALS['egw']->link('/index.php','menuaction=jinn.uiadmin.add_edit_site'),
				'text' => 'Add Site',
				'icon' => 'new'
			),
			'Browse through sites' => array(
				'link' => $GLOBALS['egw']->link('/index.php','menuaction=jinn.uiadmin.browse_egw_jinn_sites'),
				'text' => 'Browse through sites',
				'icon' => 'browse'
			),
			'Load site conf from file' => array(
				'link' => $GLOBALS['egw']->link('/index.php','menuaction=jinn.uiadmin.import_egw_jinn_site'),
		...
		if($GLOBALS['local_bo']->site['site_id'])
		{
			$site = Array(
				'Save site conf to file' => array(
					'link' => $GLOBALS['egw']->link('/index.php','menuaction=jinn.uiadmin.export_site&where_key=site_id&where_value='.$GLOBALS['local_bo']->site['site_id']),
		...
		if($GLOBALS['local_bo']->site_object['object_id'])
		{
			$object = Array(
				'Edit this Site Object' => array(
					'link' => $GLOBALS['egw']->link('/index.php','menuaction=jinn.uiadmin.edit_this_jinn_site_object'),
