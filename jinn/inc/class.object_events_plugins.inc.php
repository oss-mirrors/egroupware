<?php
   ...
   */
   class object_events_plugins
   {
	  ...
	  */
	  function object_events_plugins()
	  {
	  ...
	  */
	  function include_plugins()
	  {
		 global $local_bo;
		 $local_bo=$this;
		 if ($handle = opendir(PHPGW_SERVER_ROOT.'/jinn/object_events_plugins')) {
			...
			{ 
			   if (substr($file,0,7)=='plugin.')
			   {
				  ...
				  include_once(PHPGW_SERVER_ROOT.'/jinn/object_events_plugins/'.$file);
			   }
			}
			closedir($handle); 
		 }
	  ...
	  */
	  ...
	  @param string $eventtype 
	  */
	  function get_default_plugin($eventtype)
	  {
		 $i=1;
		 if (count($this->plugins>0))
		 {	
