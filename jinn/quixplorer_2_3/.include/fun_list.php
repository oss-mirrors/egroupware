...
//		echo $item."\" onclick=\"javascript:Toggle(this);\"></TD>\n";
	// Icon + Link
		echo "<TD nowrap>";
		/*if($link!="") */ echo"<A HREF=\"".$link."&a_target={$GLOBALS[a_target]}\" TARGET=\"".$target."\">";
		//else echo "<A>";
...
//		echo "<TD>".parse_file_size(get_file_size($dir,$item))."</TD>\n";
	...
	// Modified
		echo "<TD nowrap>".parse_file_date(get_file_date($dir,$item))."</TD>\n";
	// Permissions
		echo "<TD>";
//		if($allow) {
...
//		if($allow) echo "</A>";
	...
	echo '<TD><a href="" onclick="window.opener.document.'.$GLOBALS[a_target].'=\'/'.$dir.'/'.$item.'\';window.close();">'.$GLOBALS["messages"]["select"].'</a>';
		echo "\n<TABLE>\n";
...
function list_dir($dir) {			// list directory contents
	...
	// PARENT DIR
	echo "<TD><A HREF=\"".make_link("list",$dir_up,NULL)."&a_target={$GLOBALS[a_target]}\">";
...
*/	
	...
	// Table Header
	echo "<TR>";
//	echo "<TD colspan=\"7\"><HR></TD></TR><TR><TD WIDTH=\"2%\" class=\"header\">\n";
...
//	echo "<TD class=\"header\"></TD>";
	...
	echo "</TR>\n<TR><TD colspan=\"7\"><HR size=1 noshade></TD></TR></FORM></TABLE>\n";
