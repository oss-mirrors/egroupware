...
?>
<tr>
   <!--   <td align="left" style="background-color:<?=$rowcolor?>;padding:0px 0px 0px 0px"><input style="border-style:none;" type="checkbox" name="{colfield_check_name}" value="{colfield_check_val}"/></td>-->

   <td align="left" style="background-color:<?=$rowcolor?>;padding:0px 0px 0px 0px"><a title="<?=lang('Edit')?>" href="javascript:void(0);" onclick="m2o_edit_record('<?=$this->m2o_arr[id]?>','<?=$this->m2o_arr[object_conf]?>','<?=$linked_rec[where_string]?>','<?=$this->xmlhttp_get_m2o_link?>&obj_conf=<?=$this->m2o_arr[object_conf]?>&where_string=<?=$linked_rec[where_string]?>','<?=$this->xmlhttp_save_m2o_link?>&obj_conf=<?=$this->m2o_arr[object_conf]?>&where_string=<?=$linked_rec[where_string]?>')"><img width="16" src="<?=$this->img_edit?>" alt="<?=lang('edit')?>" /></a></td>

<!--<td align="left" style="background-color:<?=$rowcolor?>;padding:0px 0px 0px 0px"><a title="<?=lang('copy')?>" href="javascript:void(0)" onClick="return window.confirm('{colfield_lang_confirm_copy_one}')"><img width="19" src="<?=$this->img_copy?>" alt="<?=lang('copy')?>" /></a></td>-->

<td align="left" style="background-color:<?=$rowcolor?>;padding:0px 0px 0px 0px"><a title="<?=lang('delete')?>" href="javascript:void(0)" onClick="m2o_delete_record('<?=$this->m2o_arr[id]?>','<?=$this->xmlhttp_delete_m2o_link?>&where_string=<?=$linked_rec[where_string]?>');//return window.confirm('<?=lang('Delete this record?')?>');"><img width="16" src="<?=$this->img_delete?>" alt="<?=lang('delete')?>" /></a></td>
