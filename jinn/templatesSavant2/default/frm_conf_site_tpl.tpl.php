...
<script type="text/javascript" >
   ...
   //count current site profiles
   var numtab = 2+<?=$num_dom?>;
   
   var tab = new Tabs(numtab,'activetab','inactivetab','tab','tabcontent','','','tabpage');
   ...
   {
		 numtab + 1 ;
		 tab = new Tabs(numtab,'activetab','inactivetab','tab','tabcontent','','','tabpage');
			   ...
			   <table style="border-spacing: 15px;">
				  <tr>
					 <td ><?=lang('Database type')?></td><td >
						...
						?>
						<select id="DOMdb_type<?=$i?>" name="DOMdb_type<?=$i?>" onchange="dbTypeChange(this,'dev');">
						   <option value="egw" <?=$db_type_egw?>>This eGroupware (<?=lang('Only MySQL is supported')?>)</option>
						   <option value="mysql" <?=$db_type_mysql?>>MySQL</option>
						   <option value="pgsql" <?=$db_type_pgsql?>>PostgreSQL (<?=lang('Very experimental')?>)</option>
						</select>
					 </td>
				  </tr>
				  <tr><td><?=lang('Database name')?></td><td ><input name="DOMdb_name<?=$i?>" id="DOMdb_name<?=$i?>" size="30"  value="<?=$domain['db_name']?>" type="text"></td></tr>
				  <tr><td ><?=lang('Database hostname')?></td><td ><input name="DOMdb_host<?=$i?>" id="DOMdb_host<?=$i?>" size="30"  value="<?=$domain['db_host']?>" type="text"></td></tr>
				  <tr><td ><?=lang('Database username')?></td><td ><input name="DOMdb_user<?=$i?>" id="DOMdb_user<?=$i?>" size="30"  value="<?=$domain['db_user']?>" type="text"></td></tr>
				  <tr><td ><?=lang('Database password')?></td><td ><input name="DOMdb_password<?=$i?>" id="DOMdb_password<?=$i?>" size="30"  value="<?=$domain['db_password']?>" type="text"></td></tr>
				  <tr><td ><?=lang('Upload path')?></td><td ><input name="DOMupload_path<?=$i?>" size="80"  value="<?=$domain['upload_path']?>" type="text"></td></tr>
				  <tr><td ><?=lang('Upload URL')?></td><td ><input name="DOMupload_url<?=$i?>" size="80"  value="<?=$domain['upload_url']?>" type="text"></td></tr>
			   </table>
			</div>
			<?php endforeach?>
