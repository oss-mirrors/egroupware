   ...
   */
   ...
   class db_fields_plugin_imagepath
   {
	  ...
	  {	
			...
			***************************************/
			...
			for($i=1;$i<=$num_input;$i++)
			{
			   $input.='<tr><td colspan="3" '.$cell_style.'>';
					 if($num_input==1) 
					 {
						$input .=lang('add image').'<input class="egwbutton"  type="file" name="IMG_SRC'.$field_name.$i.'">';
					 }
					 else
					 {
						$input.=lang('add image %1', $i).' <input class="egwbutton"  type="file" name="IMG_SRC'.$field_name.$i.'">';
					 }
		 ...
		 {
			$input.= '<table>';
			   $input.='<tr><td>'.lang('Add your ZIP-file with images here').'<input  class="egwbutton" type="file" name="IMG_ZIP'.$field_name.'" value=""></td></tr>';
			   $input.='</table>';
		 }
	  ...
	  function on_save_filter($key, $HTTP_POST_VARS,$HTTP_POST_FILES,$config)
	  {
		 ...
		 if (count($images_to_delete)>0){
			...
			$images_org=explode(';',$HTTP_POST_VARS['IMG_ORG'.$key]);
		 ...
		 {
			$image_path_new.=$HTTP_POST_VARS['IMG_ORG'.$key];
		 }
