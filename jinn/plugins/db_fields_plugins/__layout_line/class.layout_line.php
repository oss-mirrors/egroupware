<?php
   ...
   */
   class db_fields_plugin_layout_line
   {
	  /**
	  * Constructor
	  */
	  function db_fields_plugin_layout_line()
	  {
	  ...
	  */
	  function on_save_filter($key, $HTTP_POST_VARS,$HTTP_POST_FILES,$config)
	  {
		 ...
		 $plug_conf_arr = $this->local_bo->plug->registry->plugins['layout_line']['config2'];
		 ...
		 $helper_id = $this->local_bo->plug->registry->plugins['layout_line']['helper_fields_substring'];
	  ...
	  */
	  function listview_read($value,$config,$where_val_enc)
	  {
		 ...
		 $input=$this->tplsav2->fetch('layout_line.listview_read.tpl.php');
		 return $input;
	  }
	   ...
	   */
	   function formview_read($value,$config)
	   {
		 ...
		 $input=$this->tplsav2->fetch('layout_line.formview_read.tpl.php');
		 return $input;
	   }
	  ...
	  * @note we dont use this function , its ment for custom configurations, it could usefull:(
		 ...
		 */
		 function config_dialog($config,$form_action)
		 {
			...
			$this->tplsav2->display('layout_line.config.tpl.php');
		 }
	  }	
   ?>
