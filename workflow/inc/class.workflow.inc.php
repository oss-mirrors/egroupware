<?php
	...
	class workflow
	{
		...
		{
			// check version
			if (substr($GLOBALS['egw_info']['apps']['workflow']['version'],0,6) != '1.3.00')
			{
		...
		function fill_proc_bar($proc_info)
		{
			...
			{
				$start_stop = '<td><a href="'. $GLOBALS['egw']->link('/index.php', 'menuaction=workflow.ui_adminactivities.form&p_id='. $proc_info['wf_p_id'] .'&deactivate_proc='. $proc_info['wf_p_id']) .'"><img border ="0" src="'. $GLOBALS['egw']->common->image('workflow', 'stop') .'" alt="'. lang('stop') .'" title="'. lang('stop') .'" />'.lang('stop').'</a></td>';
			}
			elseif ($proc_info['wf_is_valid'] == 'y')
			{
				$start_stop = '<td><a href="'. $GLOBALS['egw']->link('/index.php', 'menuaction=workflow.ui_adminactivities.form&p_id='. $proc_info['wf_p_id'] .'&activate_proc='. $proc_info['wf_p_id']) .'"><img border ="0" src="'. $GLOBALS['egw']->common->image('workflow', 'refresh2') .'" alt="'. lang('activate') .'" title="'. lang('activate') .'" />'.lang('activate').'</a></td>';
			}
			else
			{
				$start_stop = '';
			...
			$this->t->set_var(array(
				...
				'img_validity'			=> $GLOBALS['egw']->common->image('workflow', $dot_color.'_dot'),
				'alt_validity'			=> $alt_validity,
				'start_stop'			=> $start_stop,
				'link_admin_activities'		=> $GLOBALS['egw']->link('/index.php', 'menuaction=workflow.ui_adminactivities.form&p_id='. $proc_info['wf_p_id']),
				'img_activity'			=> $GLOBALS['egw']->common->image('workflow', 'Activity'),
				'link_admin_processes'		=> $GLOBALS['egw']->link('/index.php', 'menuaction=workflow.ui_adminprocesses.form&p_id='. $proc_info['wf_p_id']),
				'img_change'			=> $GLOBALS['egw']->common->image('workflow', 'change'),
				'link_admin_shared_source'	=> $GLOBALS['egw']->link('/index.php', 'menuaction=workflow.ui_adminsource.form&p_id='. $proc_info['wf_p_id']),
				'img_code'			=> $GLOBALS['egw']->common->image('workflow', 'code'),
				'link_admin_export'		=> $GLOBALS['egw']->link('/index.php', 'menuaction=workflow.workflow.export&p_id='. $proc_info['wf_p_id']),
				'link_admin_roles'		=> $GLOBALS['egw']->link('/index.php', 'menuaction=workflow.ui_adminroles.form&p_id='. $proc_info['wf_p_id']),
				'img_roles'			=> $GLOBALS['egw']->common->image('workflow', 'roles'),
				'link_graph'			=> $GLOBALS['egw']->link('/index.php', 'menuaction=workflow.ui_adminactivities.show_graph&p_id=' . $proc_info['wf_p_id']),
				'img_process'			=> $GLOBALS['egw']->common->image('workflow', 'Process'),
				'link_save_process'		=> $GLOBALS['egw']->link('/index.php', 'menuaction=workflow.ui_adminprocesses.save_process&id='. $proc_info['wf_p_id']),
				'img_save'			=> $GLOBALS['egw']->common->image('workflow', 'save'),
				'link_compile'			=> $GLOBALS['egw']->link('/index.php', array(
					'menuaction'	=> 	'workflow.ui_adminactivities.form',
					'p_id'		=>	$proc_info['wf_p_id'],
					'compile'	=>	true,
				)),
				'img_compile'			=> $GLOBALS['egw']->common->image('workflow', 'compilation'),
			));
		...
		function act_icon($type, $interactive)
		{
			...
			return '<img src="'. $GLOBALS['egw']->common->image('workflow', $ic) .'" alt="'. lang($type) .'" title="'. lang($type) .'" />';
		}
