...
#LK    }
	...
	function updateScript ($connection) 
	{
		...
		$version	= $GLOBALS['phpgw_info'][apps][felamimail][version];
...
#LK    if (!AppSession::isActiveScript($this->name) &&
#LK        ($default->update_activate_script || 
#LK         !$default->allow_multi_scripts ||
#LK         AppSession::getNumScripts() == 0)) {
#LK	if (!$connection->activatescript($this->name)) {
#LK	    $this->errstr = 'updateScript: activatescript failed: ' . $connection->errstr;
#LK	    return false;
#LK	}
#LK    }
...
#LK    if (!AppSession::doListScripts()) {
#LK        $this->errstr = 'updateScript: AppSession::doListScripts failed: ' . AppSession::getError();
#LK        return false;
#LK    }
