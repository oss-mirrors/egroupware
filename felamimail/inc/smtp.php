   ...
   function attachFiles ($fp) {
      ...
      if (isMultipart()) {
         ...
         while (list($localname, $remotename) = each($attachments)) {
            ...
            $fileinfo = fopen ($phpgw_info["server"]["temp_dir"] . $sep . $localname.'.info', 'r');
            ...
            $file = fopen ($phpgw_info["server"]["temp_dir"] . $sep . $localname, 'r');
   ...
   function write822Header ($fp, $t, $c, $b, $subject, $more_headers) {
	 ...
	 if(is_array($more_headers)) {
	    ...
	    while(list($h_name, $h_val) = each($more_headers)) {
         ...
         $header .= "X-Mailer: SquirrelMail for PHPGroupware (version $version)\r\n"; // Identify SquirrelMail
...
//mkorff@vpoint.com.br: added "" around username
      $from_addr = '"'.$phpgw_info["user"]["fullname"] . '"<' . $phpgw_info['user']['preferences']['email']["address"] . '>';
//
//mkorff@vpoint.com.br: impossible to reach ... $from_addr has just been set.
      if (!$from_addr)
         $from_addr = "$popuser@$domain";

//mkorff@vpoint.com.br: alternative proposal for computing from_addr:
      $from_addr = $phpgw_info['user']['preferences']['email']["address"];
      if (!$from_addr)
         $from_addr = "$popuser@$domain";
...
//mkorff@vpoint.com.br: removed <> around 'to'
         fputs($smtpConnection, "RCPT TO: $to[$i] \r\n");
         $tmp = fgets($smtpConnection, 1024);
         errorCheck($tmp, $smtpConnection);
      }
      for ($i = 0; $i < count($cc); $i++) {
//mkorff@vpoint.com.br: removed <> around 'cc'
         fputs($smtpConnection, "RCPT TO: $cc[$i] \r\n");
         $tmp = fgets($smtpConnection, 1024);
         errorCheck($tmp, $smtpConnection);
      }
      for ($i = 0; $i < count($bcc); $i++) {
//mkorff@vpoint.com.br: removed <> around 'bcc'
	 ...
	 $lines .= $line;
      ...
      if ($status == 0) {
      	global $phpgw;
			...
			echo parse_navbar();
//mkorff@vpoint.com.br: 27.6.2001
