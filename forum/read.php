...
if($pos != 0) {
 ...
 if(!$phpgw->db->query("insert into f_threads (postdate,pos,thread,depth,main,parent,cat_id,for_id,author,subject,email,host,stat) VALUES ("
                  . "'$datetime','$pos','$thread','$depth','$next_f_body_id','" . addslashes($msg) . "','"
                  . "$cat','$for','$author','" . addslashes($subject) . "','$email','$host','$stat')"))
                  { echo "echo <h1>NNING this</H1>";
                     $phpgw->common->phpgw_exit();  }
 
  if(!$phpgw->db->query("update f_threads set n_replies = n_replies+1 where thread='$thread'"))
   {echo "not berjaya second";
   $phpgw->common->phpgw_exit(); }


  if(!$phpgw->db->query("insert into f_body (cat_id,for_id,message) VALUES ('$cat','$for','" . addslashes($message) . "')"))
  { 
  	echo $phpgw->db->haltmsg(emm);
...
// From tnctions.inc.php don't know what it does << TODO
